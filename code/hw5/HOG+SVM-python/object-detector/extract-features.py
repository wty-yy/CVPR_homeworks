...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', '--pospath', help="Path to positive images",
                        default='../data/dataset/PNGImages/pos', required=False)
    parser.add_argument('-n', '--negpath', help="Path to negative images",
                        default='../data/dataset/PNGImages/neg', required=False)
    ...
    for im_path in glob.glob(os.path.join(pos_im_path, "*")):
        ...
        if des_type == "HOG":
            fd = feature.hog(im, orientations=orientations, pixels_per_cell=pixels_per_cell,
                             cells_per_block=cells_per_block, visualize=visualize)
            # fd = feature.hog(im, orientations=9, pixels_per_cell=(8, 8), cells_per_block=(3, 3), visualize=False)
